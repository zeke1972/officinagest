...
// InitDB inizializza il database BoltDB
...
func (db *DB) GetFornitore(id int) (*Fornitore, error) {
	...
	err := db.View(func(tx *bolt.Tx) error {
		...
		data := b.Get(itob(id))
		if data == nil {
			return fmt.Errorf("fornitore non trovato")
		}

		var err error
		f, err = FromJSONFornitore(data)
		return err
	})
...
func (db *DB) DeleteFornitore(id int) error {
	return db.Update(func(tx *bolt.Tx) error {
		...
		bFornitori := tx.Bucket(BktFornitori)
		if bFornitori.Get(itob(id)) == nil {
			return fmt.Errorf("fornitore #%d non trovato", id)
		}

		bPrimaNota := tx.Bucket(BktPrimaNota)

		// Elimina tutti i movimenti associati al fornitore
		var movimentiToDelete []int
		bPrimaNota.ForEach(func(k, v []byte) error {
			mov, err := FromJSONMovimentoPrimaNota(v)
			if err == nil && mov.FornitoreID == id {
				movimentiToDelete = append(movimentiToDelete, mov.ID)
			}
			return nil
		})
		...
		return bFornitori.Delete(itob(id))
	})
}
